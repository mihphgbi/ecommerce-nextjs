...
The dashboard in this project is the seller/agent management area under `/agent-management`. It is built with the Next.js App Router, client-side React components, Ant Design layout controls, Redux Toolkit async actions, and route handlers backed by Prisma and MongoDB.

The dashboard currently focuses on product management:

- Add a product.
- View the signed-in agent's product list.
- Edit an existing owned product.
- Delete an owned product after confirmation.
- Open a placeholder setting page.
...
- Allowed: `session.user.isAgent === true` or the string value `'true'`.
- Unauthenticated users are redirected to `/sign-in?callbackUrl={currentPath}`.
- Authenticated users without the agent flag are redirected to `/`.
...
| `/agent-management/setting` | `src/app/agent-management/setting/page.tsx` | Placeholder agent setting page. |

## Layout

The dashboard uses Ant Design `Layout`, `Sider`, `Header`, `Content`, `Menu`, `Spin`, and `Button`.
...
- `Setting`, linked to `/agent-management/setting`.
...
| `product_type_id` | string | Yes | Selected from `GET /api/product-types`. |
| `quality` | number | Yes | Quantity-like inventory field. The app currently uses the name `quality`. |
| `sold_items` | number | Yes | Numeric count used for sales sorting and dashboard display. |
...
The product list page dispatches `getProductData({agent_only: true})` and reads products from the Redux product slice. The API verifies the current session is an agent and returns only products owned by that agent.
...
The product table uses `rowKey="id"`.
...
The Redux actions call relative URLs such as `/api/products`, so they use the current application origin in the browser.
...
| --- | --- | --- | --- |
| `GET` | `agent_only=true` | None | Returns products owned by the signed-in agent as `{ data: Product[] }`. |
| `GET` | `page`, `limit`, `product_type_id`, `type_id`, `is_sale`, `sort_by` | None | Returns public paginated products with optional filters. |
| `POST` | None | Product payload | Creates a product owned by the signed-in agent. |
| `PUT` | `id` | Product update payload | Updates the signed-in agent's product with the matching id. |
| `DELETE` | `id` | None | Deletes the signed-in agent's product with the matching id. |

Product type API:

| Method | Query | Body | Behavior |
| --- | --- | --- | --- |
| `GET /api/product-types` | None | None | Returns product types ordered by name. |
| `POST /api/product-types` | None | `{ name }` | Creates a product type for agent users. |
...
- `sold_items` is stored as an integer with a default of `0`.
- `saler_id` links each product to a `User`.
- `product_type_id` optionally links each product to a `ProductType`.
...
- The setting page is a placeholder.
...
- Add a real dashboard overview page with product count, sale product count, total stock, sold item totals, and recent products.
...
- Add stronger loading and error states to the product table.
